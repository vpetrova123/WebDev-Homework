...
const mapStarships = (input) => {
  const answer = input.map((item) => {
    if (item.cost_in_credits != "unknown") {
      return `${item.name}, manufactured by ${
        item.manufacturer
      } - cost: ${parseInt(item.cost_in_credits).toLocaleString(
        "en-US"
      )} credits`;
    } else {
...
const filterStarships = (input) => {
  const answer = input.filter((item) => item.passengers < 10 && item.crew > 1);
...
const reduceStarships = (input) => {
  ...
  return (
    "The cost of all starships is " + sum.toLocaleString("en-US") + " credits"
  );
};
